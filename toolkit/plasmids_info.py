...
import pandas as pd
from utils import run_cmd, get_locus2group, get_length_fasta
from get_gene_info import get_gene_with_regin


def get_plasmids(indir):
    result_dict = {}
    for contig in tqdm(glob(os.path.join(indir, 'regular', '*', 'contigs.fa'))):
        run_cmd("bwa index %s" % contig, dryrun=False)
...
def main(indir, roary_dir, prokka_dir, odir):
    ...
    for sample_name in plasmids_genes.keys():
        ...
        gff_pth = os.path.join(prokka_dir, "{sn}/{sn}.gff".format(sn=sample_name))
